        ...
        if 'n' == c1 {
            ...
            if 0 == kana_cnt {
                if '\'' == s.chars().nth(1)? {
                    ...
                    info!(
                        "convert_sokuonn_and_the_sound_of_the_kana_n:: n{} detected",
                        s.chars().nth(1)?
                    );
                    Some(('ん'.to_string(), 1))
                } else {
                ...
                info!(
                    "convert_sokuonn_and_the_sound_of_the_kana_n:: nn detected ({})",
                    kana_cnt
                );
                Some((
                    iter::repeat('ん').take(kana_cnt).collect::<String>(),
                    kana_cnt * 2,
                ))
            }
        } else if 0 == cnt {
            ...
            info!(
                "convert_sokuonn_and_the_sound_of_the_kana_n:: {} detected ({})",
                c1, cnt
            );
        ...
        match s.len() {
            3 | 4 => {
                if let Some(converted) = self.from_romaji_table[1].get(s) {
                    ...
                    Some(
                        converted
                            + &self.from_romaji_impl(&s.chars().skip(cnt).collect::<String>())?,
                    )
                }
            }
            2 => self.from_romaji_table[1]
                .get(s)
                .map(|converted| converted.to_string()),
            1 => self.from_romaji_table[0]
                .get(s)
                .map(|converted| converted.to_string()),
            _ => {
                let (converted, cnt) = self.convert_sokuonn_and_the_sound_of_the_kana_n(s)?;
    ...
    pub fn to_romaji(&self, input: String) -> Option<String> {
        ...
        for c in s.chars() {
            ...
            if 'っ' == prev_c {
                ...
                info!(
                    "to_romaji:: `っ` found. prev_c: {}, prev_sokuonn_count: {}",
                    prev_c, prev_sokuonn_count
                );
                continue;
            }
            ...
            info!(
                "to_romaji:: key: {}, append: {}, next_c: {}",
                key, append, next_c
            );
            if 0 != prev_sokuonn_count {
                let sokuonn = iter::repeat(append.chars().next()?)
                    .take(prev_sokuonn_count)
                    .collect::<String>();
                info!(
                    "to_romaji:: prev_sokuonn_count: {}, create sokuonn => {}",
                    prev_sokuonn_count, sokuonn
                );
                re += &sokuonn;
        ...
        info!(
            "to_romaji:: prev_sokuonn_count: {}, prev_c: {}",
            prev_sokuonn_count, prev_c
        );
        if '\0' != prev_c {
...
mod test {
    ...
    fn convert_sokuonn_and_the_sound_of_the_kana_n() {
        ...
        assert_eq!(
            Some(("ん".to_string(), 2)),
            cvt.convert_sokuonn_and_the_sound_of_the_kana_n("nnna")
        );
        assert_eq!(
            Some(("っっ".to_string(), 2)),
            cvt.convert_sokuonn_and_the_sound_of_the_kana_n("kkkoro")
        );
    }
    #[test]
    fn from_romaji() {
        let cvt = super::RomajiCvt::new();
        assert_eq!(
            Some("ありきたり".to_string()),
            cvt.from_romaji("arikitari".to_string())
        );
        assert_eq!(
            Some("んなばかな".to_string()),
            cvt.from_romaji("nnnabakana".to_string())
        );
        assert_eq!(
            Some("なんてこったい".to_string()),
            cvt.from_romaji("nanntekottai".to_string())
        );
        assert_eq!(
            Some("しったこっちゃない".to_string()),
            cvt.from_romaji("sittakottyanai".to_string())
        );
        assert_eq!(
            Some("むっ".to_string()),
            cvt.from_romaji("muxtu".to_string())
        );
        assert_eq!(
            Some("くっっころ".to_string()),
            cvt.from_romaji("kukkkoro".to_string())
        );
        assert_eq!(
            Some("さんばし".to_string()),
            cvt.from_romaji("san'basi".to_string())
        );
        assert_eq!(
            Some("なんでやねん".to_string()),
            cvt.from_romaji("nandeyanen".to_string())
        );
    }
    #[test]
    fn to_romaji() {
        ...
        let cvt = super::RomajiCvt::new();
        assert_eq!(
            Some("arikitari".to_string()),
            cvt.to_romaji("ありきたり".to_string())
        );
        assert_eq!(
            Some("nnnabakana".to_string()),
            cvt.to_romaji("んなばかな".to_string())
        );
        assert_eq!(
            Some("nanntekottai".to_string()),
            cvt.to_romaji("なんてこったい".to_string())
        );
        assert_eq!(
            Some("sittakottyanai".to_string()),
            cvt.to_romaji("しったこっちゃない".to_string())
        );
        assert_eq!(Some("muxtu".to_string()), cvt.to_romaji("むっ".to_string()));
        assert_eq!(
            Some("muxxtu".to_string()),
            cvt.to_romaji("むっっ".to_string())
        );
        assert_eq!(
            Some("kukkkoro".to_string()),
            cvt.to_romaji("くっっころ".to_string())
        );
    }
}
